output "cosmosdb_managed_identity_federated_credential" {
  value = <<-COMMAND
        ...
        ### az cli Command to establish federated identity credential ###
                ...
                --resource-group ${azurerm_resource_group.main.name} --issuer ${data.azurerm_kubernetes_cluster.main.oidc_issuer_url} 
